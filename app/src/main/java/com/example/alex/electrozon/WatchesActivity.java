...
public class WatchesActivity extends AppCompatActivity {
    ...
    private void getProducts() {
        ...
        productList.clear();
        try {
            Cursor cr = dbHomePage.rawQuery("SELECT * FROM watchesproduct1;", null);
            if (cr != null) {
                while (cr.moveToNext()) {
    ...
    public void addToCartItem(int index) {
        Intent intent = new Intent(WatchesActivity.this, CartActivity.class);
    ...
    public void viewInfo(int index) {
        Intent intent = new Intent(WatchesActivity.this, InfoActivity.class);
        intent.putParcelableArrayListExtra("ITEMS", productList);
        //intent.putExtra("ID", productId);
        intent.putExtra("index", index);
        startActivity(intent);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(item.getItemId()==R.id.action_add){
            Intent intent = new Intent(WatchesActivity.this, AddWatchesActivity.class);
            startActivity(intent);
            return true;
        }
        if(item.getItemId()==R.id.action_home){
            Intent intent = new Intent(WatchesActivity.this, HomeActivity.class);
            startActivity(intent);
            return true;
        }
